...
export default function CareersPage() {
  return (
    <div id="top">
      {/* 1 — HERO */}
      <section aria-label="Careers introduction" className="border-b-[3px] border-ink bg-ink text-background">
        <div className="wrap grid gap-[clamp(28px,5vw,64px)] py-[clamp(72px,10vw,120px)] lg:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)] lg:items-center">
          <Reveal className="mx-auto w-full max-w-[420px] lg:mx-0">
            <div className="border-[3px] border-orange shadow-[10px_10px_0_var(--color-orange)]">
              <picture>
          ...
          <div>
            <Reveal as="p" className="mono-label inline-block border-2 border-orange px-3 py-1.5 text-orange">
              Careers // Hey We Market
            </Reveal>
            <Reveal
              as="h1"
              delay={1}
              className="mt-5 text-[clamp(2.6rem,8vw,5.5rem)] font-black uppercase leading-[0.92] tracking-[-0.03em]"
            >
            ...
            <Reveal delay={3} className="mt-8">
              <a
                href="#opportunities"
                className="inline-flex items-center gap-2.5 border-[3px] border-background bg-orange px-7 py-4 font-heading text-[15px] font-extrabold uppercase tracking-[0.02em] text-ink shadow-[6px_6px_0_var(--color-background)] transition-transform duration-150 hover:translate-x-[2px] hover:translate-y-[2px] hover:shadow-[3px_3px_0_var(--color-background)]"
              >
      ...
      {/* 2 — WHY JOIN */}
      <section aria-label="Why join us" className="section-pad border-b-[3px] border-ink">
        <div className="wrap">
          <Reveal as="h2" className="max-w-[18ch] text-balance text-[clamp(2rem,6vw,3.6rem)] font-extrabold uppercase">
            ...
            {VALUES.map((v, i) => {
              ...
              return (
                <Reveal
                  ...
                  className="flex flex-col gap-3 border-[3px] border-ink bg-card p-5 shadow-brutal-sm"
                >
                  <span className="grid size-11 place-items-center border-2 border-ink bg-orange text-ink">
                    <Icon className="size-5" />
                  </span>
                  <b className="font-heading text-[1.05rem] font-extrabold uppercase leading-tight">{v.title}</b>
      ...
      {/* 3 — OPPORTUNITIES */}
      <section id="opportunities" aria-label="Opportunities" className="section-pad border-b-[3px] border-ink bg-muted">
        <div className="wrap">
          <Reveal as="p" className="mono-label inline-block border-2 border-ink bg-card px-3 py-1.5">
            Opportunities
          </Reveal>
          <Reveal as="h2" delay={1} className="mt-4 text-[clamp(2rem,6vw,3.6rem)] font-extrabold uppercase">
            Find your place.
          </Reveal>
          ...
          <div className="mt-[clamp(32px,5vw,56px)] grid gap-5 md:grid-cols-2">
            {JOBS.map((job, i) => (
              <Reveal
                key={job.slug}
                delay={((i % 2) + 1) as 1 | 2}
                className="flex flex-col border-[3px] border-ink bg-card p-6 shadow-brutal sm:p-7"
              >
                <h3 className="text-[clamp(1.3rem,2.6vw,1.8rem)] font-extrabold uppercase leading-tight">{job.title}</h3>
                <div className="mt-3 flex flex-wrap gap-2">
                  <span className="border-2 border-ink bg-orange px-2.5 py-1 font-mono text-[10.5px] font-bold uppercase tracking-[0.06em] text-ink">
                    {job.employment}
                  </span>
                  <span className="border-2 border-ink px-2.5 py-1 font-mono text-[10.5px] font-bold uppercase tracking-[0.06em]">
                    {job.experience}
                  </span>
                </div>
      ...
      {/* 4 — FRESHERS */}
      <section aria-label="For freshers" className="section-pad border-b-[3px] border-ink bg-ink text-background">
        <div className="wrap grid gap-8 lg:grid-cols-[1.2fr_0.8fr] lg:items-center">
          <div>
            <Reveal as="h2" className="max-w-[16ch] text-[clamp(2rem,6vw,3.8rem)] font-black uppercase leading-[0.95]">
      ...
      <section id="apply" aria-label="Application form" className="section-pad scroll-mt-[80px]">
        <div className="wrap-narrow">
          <Reveal as="h2" className="text-[clamp(2rem,6vw,3.6rem)] font-extrabold uppercase">
            Ready to Join Us?
          </Reveal>
