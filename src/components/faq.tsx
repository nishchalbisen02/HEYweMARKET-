    ...
    <div className="brutal-box" data-reveal="">
      {FAQS.map((f, i) => (
        <details key={i} className="group border-t-2 border-ink first:border-t-0">
          <summary className="flex cursor-pointer list-none items-center justify-between gap-4 px-[clamp(16px,2.5vw,26px)] py-[clamp(18px,2.4vw,24px)] font-heading text-[clamp(0.95rem,1.9vw,1.2rem)]  leading-[1.15] group-open:bg-primary group-open:text-primary-foreground [&::-webkit-details-marker]:hidden">
            {f.q}
            <span
              aria-hidden="true"
          ...
          <div className="max-w-[74ch] px-[clamp(16px,2.5vw,26px)] pb-6 font-normal leading-[1.7] [&_b]:box-decoration-clone [&_b]:rounded [&_b]:bg-[color-mix(in_srgb,var(--color-warm)_15%,transparent)] [&_b]:px-[0.15em] [&_b]:font-semibold">
            {f.a}
          </div>
        </details>
      ))}
